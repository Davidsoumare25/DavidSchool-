...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const CATS = ['Tout','Électronique','Vêtements','Maison','Véhicules','Agriculture','Beauté','Sport','Autres'];
const COUNTRIES = ['Sénégal','Ghana','Côte d\'Ivoire','Mali','Burkina Faso','Guinée','Cameroun','Bénin','Togo','Niger','Nigeria','Kenya','Autre'];
const CURRENCIES = ['FCFA','GHS','NGN','KES','USD','EUR'];
const CONDITIONS = ['Neuf','Très bon état','Bon état','État correct','Pour pièces'];
const PAYMENTS = ['Mobile Money','Paiement à la livraison','Virement bancaire','Espèces en main propre'];
...
export default function App() {
  ...
  const handlePublish = async (formData, photoFiles) => {
    ...
    for (let i = 0; i < photoFiles.length; i++) {
      ...
      if (!upErr) {
        ...
        photoUrls.push({ product_id: prod.id, url: urlData.publicUrl, position: i });
      }
    }
    if (photoUrls.length > 0) {
      await supabase.from('product_photos').insert(photoUrls);
    }
  ...
  // ─── RENDER ────────────────────────────────────────────────────────────────
  if (screen === 'splash' || loading) return <Splash />;

  return (
    <div style={S.root}>
      {toast && (
        <div style={{ ...S.toast, background: { ok:'#10b981', err:'#ef4444', warn:'#f59e0b' }[toast.type] }}
      ...
      {screen === 'auth' && (
        <Auth onLogin={handleLogin} onSignup={handleSignup} notify={notify} />
      )}
      ...
      {screen === 'app' && user && profile && (
        <AppLayout
          ...
          onOpenConv={async (uid) => { await fetchConvs(uid); setScreen('messages'); setTab('msgs'); }}
      ...
      {screen === 'detail' && detail && (
        <Detail
          product={detail} user={user} profile={profile}
  ...
  return (
    <div style={S.splash}>
      <div className="fadein">
        ...
        <h1 style={S.splashTitle}>MarchéPlus</h1>
...
// ─── AUTH ─────────────────────────────────────────────────────────────────────
function Auth({ onLogin, onSignup, notify }) {
  const [mode, setMode] = useState('login');
  const [f, setF] = useState({ name:'', email:'', phone:'', country:'Sénégal', city:'', password:'', confirm:'' });
  ...
  const validate = () => {
    ...
    if (f.password.length < 6) e.password = '6 caractères minimum';
    if (mode === 'signup') {
      if (!f.name.trim()) e.name = 'Requis';
      if (!f.phone.trim()) e.phone = 'Requis';
      if (f.password !== f.confirm) e.confirm = 'Mots de passe différents';
    }
    setErr(e);
  ...
  return (
    <div style={S.authPage}>
      <div style={S.authHero}>
        ...
        <h1 style={S.authBrand}>MarchéPlus</h1>
        <p style={S.authSub}>{mode === 'login' ? 'Bon retour !' : 'Créez votre compte gratuit'}</p>
      </div>
      <div style={S.authCard}>
        ...
        {mode === 'signup' && <>
          ...
          <FI label="Téléphone *" v={f.phone} onChange={v => set('phone', v)} ph="+221 77 000 0000" type="tel" err={err.phone} />
        ...
        <FI label="Mot de passe *" v={f.password} onChange={v => set('password', v)} ph="6 caractères minimum" type="password" err={err.password} />
  ...
  return (
    <div style={S.layout}>
      <div style={S.content}>
        ...
        {tab === 'msgs'    && <MsgsTab    userId={user.id} convs={convs} onOpen={(id) => { /* handled in parent */ }} fetchConvs={fetchConvs} />}
      ...
      <nav style={S.nav}>
        {[
          ...
          { id:'msgs',    Icon:ChatIco,   label:'Messages', badge:unread },
    ...
    <div style={S.screen}>
      <div style={S.hHead}>
        <div>
          ...
          <h2 style={S.hName}>{profile?.name?.split(' ')[0] || 'Vous'}</h2>
        </div>
      ...
      {list.length === 0
        ? <Empty ico="🛒" title={products.length === 0 ? 'Aucune annonce' : 'Aucun résultat'}
            sub={products.length === 0 ? 'Publiez la première annonce !' : 'Essayez d\'autres mots-clés'} />
        : <>
    ...
    <div style={S.screen}>
      ...
      <div style={S.filterRow}>
        ...
        <select style={S.fSelect} value={sort} onChange={e => setSort(e.target.value)}>
          ...
          <option value="low">Prix ↑</option>
          <option value="high">Prix ↓</option>
        </select>
      </div>
      <p style={S.count}>{res.length} résultat{res.length > 1 ? 's' : ''}</p>
      {res.length === 0
        ? <Empty ico="😔" title="Aucun résultat" sub="Modifiez les filtres" />
...
function PublishTab({ user, profile, onAdd, notify, onDone }) {
  ...
  const [f, setF] = useState({
    title:'', price:'', currency:'FCFA', category:'Électronique',
    condition:'Bon état', description:'', location: profile?.city || '',
    phone: profile?.phone || '', payments:[],
  });
  const [err, setErr]   = useState({});
  ...
  return (
    <div style={S.screen}>
      ...
      <div style={S.stepBar}>
        {[1,2,3].map(s => (
          <div key={s} style={{ display:'flex', alignItems:'center', flex: s < 3 ? 1 : 'unset' }}>
            <div style={{ ...S.dot, ...(step >= s ? S.dotOn : {}) }}>{step > s ? '✓' : s}</div>
      ...
      {step === 1 && <>
        ...
        <div style={S.photoGrid}>
          {previews.map((src, i) => (
            <div key={i} style={S.pThumb}>
              ...
              <button style={S.pDel} onClick={() => removePhoto(i)}>✕</button>
        ...
        <FI label="Titre *" v={f.title} onChange={v => set('title', v)} ph="Ex: iPhone 14, Robe wax, Moto..." err={err.title} />
        ...
        <FS label="Catégorie" v={f.category} onChange={v => set('category', v)} opts={CATS.slice(1)} />
        <FS label="État" v={f.condition} onChange={v => set('condition', v)} opts={CONDITIONS} />
      ...
      {step === 2 && <>
        ...
        <FI label="Numéro de contact *" v={f.phone} onChange={v => set('phone', v)} ph="+221 77 000 0000" type="tel" err={err.phone} />
        <p style={S.lbl}>Modes de paiement *</p>
        {PAYMENTS.map(p => (
          <button key={p} style={{ ...S.payOpt, ...(f.payments.includes(p) ? S.payOptOn : {}) }} onClick={() => togglePay(p)}>
            <span style={{ ...S.chk, ...(f.payments.includes(p) ? S.chkOn : {}) }}>{f.payments.includes(p) ? '✓' : ''}</span>
            {p}
          </button>
        ))}
      ...
      {/* ÉTAPE 3 */}
      {step === 3 && <>
        <h3 style={{ fontWeight:800, marginBottom:14 }}>Vérifiez votre annonce</h3>
        ...
        <div style={S.prevBox}>
          ...
          <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
            {f.payments.map(p => <Tag key={p} green>✓ {p}</Tag>)}
          </div>
          <p style={{ color:'#6b7280', fontSize:13, marginTop:8 }}>📞 {f.phone}</p>
          {previews.length > 1 && <p style={{ color:'#9ca3af', fontSize:12 }}>+{previews.length - 1} autre(s) photo(s)</p>}
        </div>
        <div style={{ display:'flex', gap:10, marginTop:14 }}>
          ...
          <button style={{ ...S.btnP, flex:2, marginTop:0 }} onClick={publish} disabled={busy} className="press">
            {busy ? <span className="spin">↻</span> : '🚀 Publier'}
          </button>
        </div>
      </>}
    </div>
            ...
            return (
              <div key={c.id} style={S.convRow} className="press">
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ fontWeight:700, margin:'0 0 2px' }}>{other?.name}</p>
...
function ProfileTab({ user, profile, products, onLogout, onUpdate, notify, onOpen, onDelete }) {
  ...
  const [f, setF]           = useState({ name: profile?.name || '', phone: profile?.phone || '', country: profile?.country || 'Sénégal', city: profile?.city || '' });
  ...
  return (
    <div style={S.screen}>
      <div style={S.profHero}>
        ...
        <h2 style={{ margin:'12px 0 4px', fontSize:22, fontWeight:800 }}>{profile?.name}</h2>
        <p style={{ color:'#6b7280', margin:0, fontSize:14 }}>📍 {profile?.city || profile?.country || '—'}</p>
        <div style={S.statsRow}>
          ...
          <div style={{ textAlign:'center', flex:1, padding:'12px 0', borderLeft:'1px solid #f0f0f0' }}>
            ...
            <p style={{ margin:0, fontSize:12, color:'#9ca3af' }}>Membre</p>
          </div>
        </div>
      </div>

      {!edit ? <>
        {[['✉️','Email',user.email],['📞','Téléphone',profile?.phone||'—'],['🌍','Pays',profile?.country||'—'],['📍','Ville',profile?.city||'—']].map(([ic,lb,vl]) => (
          <div key={lb} style={S.infoRow}><span style={{ fontSize:20 }}>{ic}</span><div><p style={{ margin:0, fontSize:12, color:'#9ca3af' }}>{lb}</p><p style={{ margin:0, fontWeight:600 }}>{vl}</p></div></div>
        ))}
        <button style={{ ...S.btnS, marginTop:12 }} onClick={() => setEdit(true)}>✏️ Modifier le profil</button>
      </> : <>
        <FI label="Nom" v={f.name} onChange={v => setF(p => ({ ...p, name: v }))} />
        <FI label="Téléphone" v={f.phone} onChange={v => setF(p => ({ ...p, phone: v }))} type="tel" />
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button style={{ ...S.btnP, flex:1, marginTop:0 }} onClick={save} disabled={busy}>
            {busy ? <span className="spin">↻</span> : 'Enregistrer'}
          </button>
        </div>
      </>}
      ...
      {mine.length > 0 && <>
        ...
        {mine.map(p => (
          <div key={p.id} style={S.myPRow}>
            ...
            <button style={S.delBtn} onClick={() => { if (window.confirm('Supprimer ?')) onDelete(p.id); }}>🗑️</button>
          </div>
        ))}
      </>}

      <button style={{ ...S.btnDanger, marginTop:24, marginBottom:32 }} onClick={() => { if (window.confirm('Se déconnecter ?')) onLogout(); }}>
        Déconnexion
      </button>
    </div>
  );
...
// ─── DETAIL ───────────────────────────────────────────────────────────────────
function Detail({ product: p, user, profile, onBack, onChat }) {
    ...
    <div style={S.detPage}>
      ...
      <div style={S.detBody}>
        ...
        <div style={S.detSec}>
          ...
          <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
            {p.payments?.map(pm => <Tag key={pm} green>✓ {pm}</Tag>)}
          </div>
        </div>
        ...
        {!isOwner && user && (
          <div style={{ display:'flex', gap:10, marginTop:16, paddingBottom:24 }}>
            <a href={`tel:${p.phone}`} style={{ ...S.btnS, flex:1, textAlign:'center', textDecoration:'none', display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}>
              📞 Appeler
            </a>
            <button style={{ ...S.btnP, flex:2, marginTop:0 }} onClick={onChat} className="press">
              💬 Message
            </button>
          </div>
        )}
        {!user && <p style={{ color:'#9ca3af', textAlign:'center', fontSize:14, marginTop:16 }}>Connectez-vous pour contacter le vendeur</p>}
        {isOwner && <div style={{ background:'#fef3c7', borderRadius:12, padding:'12px 16px', border:'1px solid #fde68a', marginTop:16, textAlign:'center' }}><p style={{ margin:0, color:'#92400e' }}>✏️ C'est votre annonce</p></div>}
      </div>
    </div>
  );
}
  ...
  return (
    <div style={S.chatPage}>
      <div style={S.chatHead}>
        <button style={S.backInline} onClick={onBack}>←</button>
        {other && <Av profile={other} size={38} />}
        <div>
          <p style={{ margin:0, fontWeight:700, fontSize:15 }}>{other?.name || '...'}</p>
          <p style={{ margin:0, fontSize:11, color:'#9ca3af' }}>{other?.city || other?.country || ''}</p>
        </div>
      </div>
      <div style={S.chatMsgs}>
        {msgs.length === 0 && (
          <p style={{ textAlign:'center', color:'#9ca3af', padding:'32px 0', fontSize:14 }}>Démarrez la conversation !</p>
        )}
        {msgs.map((m, i) => {
...
function Av({ profile, size = 40, override }) {
  const src = override || profile?.avatar_url;
  const ini = profile?.name?.split(' ').map(n => n[0]).join('').slice(0,2).toUpperCase() || '?';
  const bgs = ['#FF6B35','#FF9E1B','#2EC4B6','#E71D36','#4CAF50','#9C27B0'];
  const bg  = bgs[(profile?.name?.charCodeAt(0) || 0) % bgs.length];
  return src
    ? <img src={src} alt="" style={{ width:size, height:size, borderRadius:'50%', objectFit:'cover', flexShrink:0 }} />
    : <div style={{ width:size, height:size, borderRadius:'50%', background:bg, color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:800, fontSize:size*0.34, flexShrink:0 }}>{ini}</div>;
}
...
function SearchBar({ q, setQ, autoFocus }) {
  return (
    <div style={S.sBar}>
      ...
      <input style={S.sInput} placeholder="Rechercher un produit..." value={q} onChange={e => setQ(e.target.value)} autoFocus={autoFocus} />
      {q && <button style={{ border:'none', background:'none', cursor:'pointer', color:'#9ca3af', fontSize:13 }} onClick={() => setQ('')}>✕</button>}
    </div>
  );
}
...
function FI({ label, v, onChange, ph, type='text', err }) {
  return (
    <div style={S.fw}>
      {label && <p style={S.lbl}>{label}</p>}
      <input style={{ ...S.inp, ...(err ? { borderColor:'#ef4444' } : {}) }}
        type={type} value={v} onChange={e => onChange(e.target.value)} placeholder={ph} />
      {err && <Err>{err}</Err>}
    </div>
  );
}
...
// ─── SVG ICONS ────────────────────────────────────────────────────────────────
const CartSvg   = ({size=48,color='#fff'}) => <svg width={size} height={size} viewBox="0 0 72 72" fill="none"><path d="M18 24h5l6 22h16l6-22h5" stroke={color} strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"/><circle cx="30" cy="52" r="3" fill={color}/><circle cx="46" cy="52" r="3" fill={color}/><path d="M23 24l3-10h20l3 10" stroke={color} strokeWidth="2.5" strokeLinecap="round" fill="none" opacity="0.7"/></svg>;
...
const S = {
  ...
  splashTitle:{color:"#fff",fontSize:38,fontWeight:900,margin:0,letterSpacing:-1},
  ...
  backInline:{background:"none",border:"none",cursor:"pointer",fontWeight:800,fontSize:20,color:"#374151",padding:"0 8px 0 0"},
